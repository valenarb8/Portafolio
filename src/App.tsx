...
function App() {
  const initialPage = (sessionStorage.getItem('activePage') as 'home' | 'about' | 'projects') || 'home';
  const initialCategory = sessionStorage.getItem('activeCategory') || null;

  const [activePage, setActivePage] = useState<'home' | 'about' | 'projects'>(initialPage);
  const [activeCategory, setActiveCategory] = useState<string | null>(initialCategory);
  
  // Show preloader only if we are initially loading the home page
  const [loading, setLoading] = useState(initialPage === 'home');
